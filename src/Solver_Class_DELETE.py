        ...
        for nn in range(N_Steps):
            ...
            while Check_Energy == "False":
                ...
                for ii in range(N_Cells):
                    ...
                    #print(   "i: %d U: %15.10f Eta: %15.10f" %( ii, U[ii], Eta[ii]) )
                ...
                for ii in range(N_Cells+1):
                    if ii==0: # Boundary condition at face 1/2
                        ...
                        a              = 1.0 + (2.0 * Gamma_F_hat * (H_hat**2.0) )/(A_F_hat) - ( Gravity * Eta_F_hat/E_F[ii]  ) * ( 1.0 + 2.0*Gamma_F_hat*(H_hat**2.0)/A_F_hat ) - 2.0 * Gravity * H_hat/E_F[ii]
                        ...
                        Eta_Epsilon1   = ( A_F_hat / B_F_hat ) * ( -b-1.0 + (  ( (b+1.0)**2.0 - 4.0 * a * c )**0.5 ) ) / ( 2.0 * a ) 
                        Eta_Epsilon2   = ( A_F_hat / B_F_hat ) * ( -b-1.0 - (  ( (b+1.0)**2.0 - 4.0 * a * c )**0.5 ) ) / ( 2.0 * a ) 
                        ...
                        #Q_check = ( 2.0*( (A_F[ii])**(2.0) ) * ( E_F[ii]-Gravity*Eta_F[ii] )   ) **(0.5)
                        Q_check = Q_F[ii] 
                ...
                for ii in range(1,N_Cells):
                    #if E[ii] - E[ii-1]>1.0e-10:
                    if E[ii] - E[ii-1] > 0.0:
                        print(" Energy %d %d %15.12f %15.12f %e" % (ii-1, ii, E[ii-1], E[ii], E[ii-1] - E[ii]))
                        Check_Energy = "False"
                        ...
                        print(" Modifying the solution %d %d %f %15.12f %15.12f" % (nn, ii, Delta_Q, Q[ii-1], Q[ii]  ) )
            ...
            while Check_Energy == "False":
                ...
                for ii in range(N_Cells+1):
                    if ii==0: # Boundary condition at face 1/2
                        ...
                        Gamma_F_hat    = (L[ii]*Gamma[ii-1] + L[ii-1]*  Gamma[ii] )/( L[ii] + L[ii-1] ) # <modify> Modify this equation for a variable area
                        H_hat          = A_F_hat / B_F_hat
                        a              = 1.0 + (2.0 * Gamma_F_hat * (H_hat**2.0) )/(A_F_hat) - (  (Gravity * Eta_F_hat)/E_F_1[ii]  ) * (1.0 + (2.0*Gamma_F_hat*(H_hat**2.0))/A_F_hat ) - 2.0 * Gravity * H_hat/E_F_1[ii]
                        b              = 2.0 - Gravity * ( 2.0 * Eta_F_hat + H_hat ) / E_F_1[ii]
                        c              = 1.0 - (Gravity * Eta_F_hat/E_F_1[ii]) - Q_F_hat_S/( 2.0 * E_F_1[ii] * (A_F_hat**2.0) )

                        if ((b+1.0)**(2.0) - 4.0 * a * c) < 0.0:
                            print(" Fatal error: negative %e %e" %( (Gravity * Eta_F_hat/E_F_1[ii]), Q_F_hat_S/( 2.0 * E_F_1[ii] * (A_F_hat**2.0) ) ))
                            print(" Fatal error: negative %e %e %e %e" %(a,b,c, (b+1)**(2.0) - 4.0 * a * c))
                            check = input(" Error: press ENTER to exit ")
                            sys.exit()
                        ...
                        #Q_check = ( 2.0*((A_F_1[ii])**(2.0)) * ( E_F_1[ii]-Gravity*Eta_F_1[ii] )   ) **(0.5)
                        Q_check = Q_F[ii] 
                ...
                for ii in range(1,N_Cells):
                    if E_1[ii] - E_1[ii-1]>1.0e-10:
                        ...
                        print(" Modifying the solution at the half time step %d %d %15.12f %15.12f %15.12f" % (nn, ii,Delta_Q, Q_1[ii-1], Q_1[ii] ) )
                        #check = input(" Press ENTER to continue ")
            ...
            if (nn%Plot2) == 0:
                ...
                TITLE1 = Ex.Output_Dir + "/" +  "Full__time_" + str(RealTime)+"_s__Repeat_" + str(repeat)
                TITLE2 = "Full results at time: " + str(RealTime) +"_s__Repeat_" + str(repeat)
                Draw.Plot_Full(3, N_Cells, X_F, Z_F, Q_1, Q_F_1, Eta_1, Eta_F_1, U_1, U_F_1, E_1, E_F_1, A_1, A_F_1, TITLE1, TITLE2)
