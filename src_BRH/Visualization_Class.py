...
class Visualization:
    ...
    def Plot_at_Cell(N, X, Z, Q, V, Eta, U, E, A, T1, T2):
        ...
        #plt.pause(0.01)
        #plt.draw
        #BRH 20171120 plt.show(block=False) # <modify> See why the execution stops when the the command gets here. 
    ...
    @staticmethod
    def Plot_Full(iii, N, X_F, Z_F, Q, Q_F, Eta, Eta_F, U, U_F, E, E_F, A, A_F, V, T1,T2):
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        for ii in range(N):
            ...
            # interpolate for volume on face (not in solver)
            if ii == 0:
                V_Arr[0]  = V[0]
                V_Arr[1]  = V[0]
            else:
                V_Arr[ii*2]    = 0.5 * (V[ii] + V[ii-1])
                V_Arr[ii*2+1]  = V[ii]
            V_Arr[N*2] = V[N-1]
...
#        sys.exit()
        ...
        #BRH 20171120 mng = plt.get_current_fig_manager()
        #BRH 20171120 mng.resize(*mng.window.maxsize())

        #plt.pause(0.01)
        #BRH 20171120 plt.draw
        #BRH 20171120 plt.show(block=False) # <modify> See why the execution stops when the the command gets here. 
        plt.show() # <modify> See why the execution stops when the the command gets here. 
        FileName = T1 +'.jpg'
        plt.savefig(FileName)
