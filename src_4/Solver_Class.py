...
#        if face_interp == 1:                
        for ii in range(1,N_Cells):
            ...
            if sup * sdn != -1: # not a convergent or divergent flow
                # flow inboth cells in the same direction
                ...
                if el['Fr'][upi] >= 1.0:
                    fa['A_F'][ii] = min(rootout)
                else:
                    fa['A_F'][ii] = max(rootout)
...
#=============================================================================
    def solve(self):
        ...
        Setting = Setting_Class.Setting()

        # for automatically selecting time step use DT_variable = 1
        DT_variable = 0 
        DT_min = Setting.DT_min
        DT_max = Setting.DT_max
        CFLu_max = Setting.CFLu_max
        CFLb_max = Setting.CFLb_max
        Fr_max = Setting.Fr_max

        depth_min = Setting.depth_min
 
        Plot_at_Cell = Setting.Plot_at_Cell # Plots the results at the cell center, every "Plot_at_Cell" steps.
        Plot_at_Face = Setting.Plot_at_Face # Plots the results at the face, every "Plot_at_Face" steps.
        ...
        # control the time advance
        time_advance = Setting.time_advance
        #time_advance = 'rk2'
        #time_advance = 'rk4'
        #time_advance = 'forward_euler'
